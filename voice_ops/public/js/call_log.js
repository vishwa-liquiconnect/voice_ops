frappe.ui.form.on("Call Log", {
	refresh(frm) {
		if (frm.doc.recording_url) {
			...
			frm.add_custom_button(__("Transcribe Recording"), function () {
				frappe.call({
					method: "voice_ops.api.call_log.transcribe_recording",
					args: { call_log_name: frm.doc.name },
					freeze: true,
					freeze_message: __("Transcribing recording..."),
					callback(r) {
						if (r.message && r.message.transcript) {
							frm.set_value("summary", r.message.transcript);
							frm.save();
							frappe.show_alert({
